...
import javax.swing.*;
 ...
 public class ShopManagementGUI { // These are the static variables that are utilized throughout the GUI code
     static InventoryManager inventory = new InventoryManager(new Product[]{});
     static SalesManager salesManager = new SalesManager();
 
     public static void main(String[] args) {  // This is to set up the main GUI frame
    ...
    String[] options = {"Add Product", "View Inventory", "Remove Product", "Back"}; // This creates a dialog box to allow the options to be displayed to the user
    int choice = JOptionPane.showOptionDialog(null, "Manage Products", "Manage",
            JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE, null, options, options[0]);

    switch (choice) {
        case 0: // Add Product. Prompts the user to enter the product name, price, and stock to add a new product to the inventory
    ...
    // This allows for a method to be created to record a sale within the application
     static void recordSale() {
         ...
         try {
             ...
             if (quantity <= 0) { // This displays a message if there is not enough stock available for the selected product
                JOptionPane.showMessageDialog(null, "Quantity must be greater than 0.");
                recordSale();
                return;
            }
             if (quantity > productToSell.getStock()) { // This displays a message if there is not enough stock available for the selected product
                 ...
                 salesManager.recordSale(sale); // This displays a message if there is not enough stock available for the selected product
